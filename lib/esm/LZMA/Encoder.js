...
import RangeEncoder from '../RangeCoder/Encoder';
import LZ from '../LZ';
...
// shortcuts
var initBitModels = RangeEncoder.initBitModels;
...
var Encoder = function() {
  ...
  this._posAlignEncoder = new BitTreeEncoder(Base.kNumAlignBits);
...
LiteralEncoder.Encoder2.prototype.getPrice = function(matchMode, matchByte, symbol) {
  ...
  if (matchMode) {
    for (; i >= 0; i--) {
      ...
      bit = (symbol >>> i) & 1;
      price += RangeEncoder.getPrice(this._encoders[((1 + matchBit) << 8) + context], bit);
      context = (context << 1) | bit;
      if (matchBit !== bit) {
        i--;
        break;
  ...
  for (; i >= 0; i--) {
    bit = (symbol >>> i) & 1;
    price += RangeEncoder.getPrice(this._encoders[context], bit);
    context = (context << 1) | bit;
  }
  return price;
};
...
var LenEncoder = Encoder.LenEncoder = function() {
  ...
  this._lowCoder = [];
  this._midCoder = [];
  this._highCoder = new BitTreeEncoder(Base.kNumHighLenBits);
...
LenEncoder.prototype.setPrices = function(posState, numSymbols, prices, st) {
  var a0 = RangeEncoder.getPrice0(this._choice[0]);
  var a1 = RangeEncoder.getPrice1(this._choice[0]);
  var b0 = a1 + RangeEncoder.getPrice0(this._choice[1]);
  var b1 = a1 + RangeEncoder.getPrice1(this._choice[1]);
  var i;
...
Encoder.prototype.getRepLen1Price = function(state, posState) {
  return RangeEncoder.getPrice0(this._isRepG0[state]) +
    RangeEncoder.getPrice0(this._isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
};
...
Encoder.prototype.getPureRepPrice = function(repIndex, state, posState) {
  ...
  if (repIndex === 0) {
    ...
    price += RangeEncoder.getPrice1(this._isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
  } else {
...
Encoder.prototype.getOptimum = function(position) {
  ...
  this._optimum[1].price = RangeEncoder.getPrice0(this._isMatch[(this._state << Base.kNumPosStatesBitsMax) + posState]) +
  ...
  var matchPrice = RangeEncoder.getPrice1(this._isMatch[(this._state << Base.kNumPosStatesBitsMax) + posState]);
  var repMatchPrice = matchPrice + RangeEncoder.getPrice1(this._isRep[this._state]);
  ...
  var normalMatchPrice = matchPrice +
    RangeEncoder.getPrice0(this._isRep[this._state]);
  ...
  while (true) {
    ...
    var curAnd1Price = curPrice +
      RangeEncoder.getPrice0(this._isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
    ...
    matchPrice = curPrice + RangeEncoder.getPrice1(this._isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
    ...
    if (!nextIsChar && matchByte !== currentByte) {
      ...
      if (lenTest2 >= 2) {
        ...
        var nextRepMatchPrice = curAnd1Price +
          RangeEncoder.getPrice1(this._isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
          RangeEncoder.getPrice1(this._isRep[state2]);
      ...
      if (lenTest < numAvailableBytesFull) {
        ...
        if (lenTest25 >= 2) {
          ...
          var curAndLenCharPrice = repMatchPrice +
            ...
            RangeEncoder.getPrice0(this._isMatch[(state25 << Base.kNumPosStatesBitsMax) + posStateNext5]) +
          ...
          var nextMatchPrice5 = curAndLenCharPrice +
            RangeEncoder.getPrice1(this._isMatch[(state25 << Base.kNumPosStatesBitsMax) + posStateNext5]);
          var nextRepMatchPrice5 = nextMatchPrice5 +
            RangeEncoder.getPrice1(this._isRep[state25]);
    ...
    if (newLen >= startLen) {
      normalMatchPrice = matchPrice +
        RangeEncoder.getPrice0(this._isRep[state]);
          ...
          if (lenTest6 < numAvailableBytesFull) {
            ...
            if (lenTest27 >= 2) {
              ...
              var curAndLenCharPrice7 = curAndLenPrice6 +
                RangeEncoder.getPrice0(this._isMatch[(state27 << Base.kNumPosStatesBitsMax) + posStateNext7]) +
              ...
              var nextMatchPrice7 = curAndLenCharPrice7 +
                RangeEncoder.getPrice1(this._isMatch[(state27 << Base.kNumPosStatesBitsMax) + posStateNext7]);
              var nextRepMatchPrice7 = nextMatchPrice7 +
                RangeEncoder.getPrice1(this._isRep[state27]);
...
Encoder.prototype.fillDistancesPrices = function() {
  ...
  for (i = Base.kStartPosModelIndex; i < Base.kNumFullDistances; i++) {
    ...
    var baseVal = ((2 | (posSlot & 1)) << footerBits);
    tempPrices[i] =
      BitTreeEncoder.reverseGetPrice(this._posEncoders,
                                                baseVal - posSlot - 1,
  ...
  for ( ; lenToPosState < Base.kNumLenToPosStates; lenToPosState++) {
    ...
    for (posSlot = Base.kEndPosModelIndex; posSlot < this._distTableSize; posSlot++) {
      this._posSlotPrices[st + posSlot] += ((((posSlot >>> 1) - 1) - Base.kNumAlignBits) << RangeEncoder.kNumBitPriceShiftBits);
    }
